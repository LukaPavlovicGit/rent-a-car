...
public class UserService {
    ...
    public UserDto updateManager(UpdateManagerDto updateManagerDto) throws IOException {
        String token = MainFrame.getInstance().getToken();
        RequestBody body = RequestBody.create(JSON, objectMapper.writeValueAsString(updateManagerDto));

        Request request = new Request.Builder()
                .url(URL + "/users/update-manager")
                .addHeader("authorization", "Bearer " + token)
                .put(body)
                .build();
        Call call = client.newCall(request);
    ...
    public UserDto updateClient(UpdateClientDto updateClientDto) throws IOException {
        String token = MainFrame.getInstance().getToken();
        RequestBody body = RequestBody.create(JSON, objectMapper.writeValueAsString(updateClientDto));

        Request request = new Request.Builder()
                .url(URL + "/users/update-client")
                .addHeader("authorization", "Bearer " + token)
                .put(body)
                .build();
        Call call = client.newCall(request);
